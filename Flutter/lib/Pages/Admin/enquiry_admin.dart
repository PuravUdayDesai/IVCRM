...
import 'add_enquiry_admin.dart';
import 'update_enquiry_admin.dart';
...
class EnquiryAdmin extends StatefulWidget{
	EnquiryAdminState createState () => EnquiryAdminState();
}

List<EnquiryClass> enquiries = [];
class EnquiryAdminState extends State<EnquiryAdmin> with TickerProviderStateMixin{
  ...
  Future<bool> missingDialog(String unit){
    return showDialog(
      ...
      builder : (c){
        return AlertDialog(
          ...
          //   ),
          // ),
          content : Text('Please add at least one $unit',style : TextStyle(fontWeight : FontWeight.bold)),
          actions : <Widget>[
            FlatButton(
              onPressed : (){
                Navigator.of(c).pop();
    ...
    if(getNewData){
      ...
      if(getNewFilterData){
        ...
        var allclients = await ApiCall.getDataFromApi(Uri.GET_CLIENT+'/owner/${CurrentUser.id}');
        var allenquirytypes = await ApiCall.getDataFromApi(Uri.GET_ENQUIRY_TYPE+'/owner/${CurrentUser.id}');  
        ...
        Card(
            ...
            child: ListTile(
                ...
                onTap: () {
                  Navigator.push(
                  	context,
                  	MaterialPageRoute(builder : (context)=> UpdateEnquiryAdmin(obj))
                  ).then((value){
                    if(value!=null){
                      if(value){
  ...
  deleteEnquiry(int id) async {
    ...
    filterList = Consumer<ThemeNotifier>(
      ...
      drawer : CustomDrawer(enquirySelected : true),
          ...
          setState((){isLoading = true;});
            ...
            var r5 = await ApiCall.getDataFromApi(Uri.GET_ENQUIRY_TYPE+"/owner/${CurrentUser.id}");
            ...
            var r6 = await ApiCall.getDataFromApi(Uri.GET_CLIENT+"/owner/${CurrentUser.id}");
            if(r6 == 'nothing'){
              setState((){isLoading = false;});
              missingDialog('CLIENT');
              return;
            }
            var r7 = await ApiCall.getDataFromApi(Uri.GET_STATUS+"/owner/${CurrentUser.id}");
            if(r7 == 'nothing'){
              setState((){isLoading = false;});
              missingDialog('STATUS');
              return;
            }
            var r8 = await ApiCall.getDataFromApi(Uri.GET_PRODUCT+"/owner/${CurrentUser.id}");
            if(r8 == 'nothing'){
              setState((){isLoading = false;});
            ...
            if(r5!='nothing' && r6!='nothing'&& r7!='nothing'&& r8!='nothing'){
              setState((){isLoading = false;});
              Navigator.push(context,MaterialPageRoute(builder : (context)=>AddEnquiryAdmin())).then((value){
                if(value!=null){
