...
class UpdateEnquiryAdmin extends StatefulWidget{
	final EnquiryClass obj;
	UpdateEnquiryAdmin(this.obj);
	_UpdateEnquiryAdminState createState() => _UpdateEnquiryAdminState();
}

class _UpdateEnquiryAdminState extends State<UpdateEnquiryAdmin> with SingleTickerProviderStateMixin {
	...
	Future<String> getEnquiryType() async{
		...
		if(getNewEnquiryTypeData){
			...
			var r = await ApiCall.getDataFromApi(Uri.GET_ENQUIRY_TYPE+"/owner/${CurrentUser.id}");
	...
	Future<String> getClients() async{
		...
		if(getNewClientData){
			...
			var json = await ApiCall.getDataFromApi(Uri.GET_CLIENT+"/owner/${CurrentUser.id}");
	...
	Widget makeEnquiryAccessCard(var obj){
		return Card(
			...
			child : ListTile(
				...
				contentPadding: EdgeInsets.only(left: 10.0, top: 10.0),
				onTap : (){},
				trailing : obj.ownerFlag ? Text('owner') : IconButton(
					icon : Icon(Icons.delete),
					onPressed : (){
